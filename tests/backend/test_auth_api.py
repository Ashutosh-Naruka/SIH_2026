...
class TestDefaults:
    """The default is OPEN, and this checks it without trusting the value the
    test suite has already overridden.
    ...
    The default flipped from closed to open: this build is a single-desk
    evaluation deployment, where the sign-in screen guarded a desk holding no
    per-account data and cost every reader a bootstrap form before the first
    quote. The gate itself is unchanged and still tested in both directions
    above -- setting DESK_REQUIRE_AUTH=1 closes every route again.
    """

    @staticmethod
    def _enforced_for(env_value: str | None) -> bool:
        """The rule from backend.main, applied to one value."""
        raw = "0" if env_value is None else env_value
        return raw.lower() not in {"0", "false", "no"}

    def test_an_unset_variable_means_the_desk_is_open(self) -> None:
        assert self._enforced_for(None) is False

    @pytest.mark.parametrize("value", ["0", "false", "False", "no", "NO"])
    def test_an_explicit_negative_also_opens_it(self, value: str) -> None:
        assert self._enforced_for(value) is False

    @pytest.mark.parametrize("value", ["1", "true", "yes", "", "anything"])
    def test_anything_else_closes_it(self, value: str) -> None:
        """Including the empty string and a typo. A misspelled value still
        closes the desk -- the ambiguity resolves towards asking for a
        password, never away from it; only an absent variable (this
        deployment's deliberate default) or an explicit negative opens it."""
    ...
    def test_the_module_uses_exactly_this_rule(self) -> None:
        ...
        assert 'os.environ.get("DESK_REQUIRE_AUTH", "0").lower() not in {' in source
