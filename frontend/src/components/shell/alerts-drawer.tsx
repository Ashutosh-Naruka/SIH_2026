...
export function AlertsDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    // This drawer used to be a plain conditional render with its own
    // one-off backdrop (z-40, 40% black) and header (no navbar background,
    // no uppercase title, a differently-styled close button) -- the only one
    // of the four right-hand panels not built on the shared modal contract
    // the quote drawer established. That is why it alone opened with no
    // animation and read as a visibly different piece of UI: it isn't
    // skipping the animation on purpose, it never had the AnimatePresence
    // wrapper the other three share. Rebuilt on the same contract as
    // settings-drawer.tsx and help-drawer.tsx -- same backdrop, same spring,
    // same header -- so all four are one component in every way that shows.
    <AnimatePresence>
      {open && (
        <>
          <motion.div
          ...
          <motion.aside
            ...
            aria-modal="true"
            aria-label="Alerts"
            className="fixed right-0 top-0 z-50 flex h-full w-120 max-w-[92vw] flex-col border-l border-border bg-surface shadow-raised"
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            exit={{ x: '100%' }}
          ...
          >
            <div className="flex h-10 shrink-0 items-center justify-between border-b border-border bg-navbar px-3 text-navbar-foreground">
              <span className="text-lead font-bold uppercase tracking-wide">Alerts</span>
              <button
                type="button"
                onClick={onClose}
