...
| Risk tolerance | 0 prices on the middle forecast, 1 on the pessimistic one. Moves the ceiling by roughly 1 to 2% on a typical lot |
...
- **Carbon Intensity**: per vessel, the A to E carbon rating the voyage would earn
...
| **Ceiling calculator** (`opt/ceiling.py`) | What is the highest fixed daily rate at which locking beats staying spot? | Horizon-blended weighted average of the forecast quantiles over the contract term (days 1 to 18 priced off the 7-day forecast, 19 to 60 off the 30-day, 61+ off the 90-day), adjusted by route basis and risk tolerance | Deliberately physics-free, it prices the *market* only. Fuel, distance and port costs belong in the voyage estimator, and mixing them here would make the walk-away price un-auditable |
...
|---|---|---|---|
| Baltic Exchange daily indices (BCI, BPI, BSI, BHSI, BDI) and time-charter averages, 2005 to 2026-08-20, scraped | 136,933 rows across 79 series in the master table | The forecast model's target and its lag/rolling/cross features; today's real daily rate | **Real, observed** |
...
5. Every external fetch is a build-time harvester writing to disk, cached, and
   offline-safe. The one exception, a rolling seven-day marine forecast that cannot
