...
The desk is **open by default** — it starts with no sign-in, because this build is a single-desk
evaluation deployment and a "create the first administrator" form in front of a demo is a password
prompt guarding nothing. The account system is fully built and one variable away.

To close it, so every route requires a real account:

```
set DESK_REQUIRE_AUTH=1
```
With it closed, a deployment holding no accounts yet offers to create the first one, an
administrator — about twenty seconds, and it explains itself.
...
| Variable | Effect |
|---|---|
| `DESK_REQUIRE_AUTH` | `1` to require a sign-in on every route. Default: the desk is open to anyone. |
